...
switch ($upgrade){
	...
	case "barracks":
        $level=$barracks;
		$name="Barracks";
		if ($mainbuilding>=5){$buildingreq=1;}else{$buildingreq=2;$buildingreqtext="Build your main building to level 5 to build a barracks";}
        break;
	case "marketplace":
        $level=$marketplace;
		$name="Marketplace";
		if ($mainbuilding>=7 and $storage>=5){$buildingreq=1;}else{$buildingreq=2;$buildingreqtext="Build your main building to level 7 and your storage to level 5 to build a marketplace";}
        break;
	case "stable":
        $level=$stable;
		$name="Stable";
		if ($mainbuilding>=7 and $barracks>=5){$buildingreq=1;}else{$buildingreq=2;$buildingreqtext="Build your main building to level 7 and your barracks to level 5 to build a stable";}
        break;
	case "wall":
        $level=$wall;
		$name="Wall";
        if ($mainbuilding>=3){$buildingreq=1;}else{$buildingreq=2;$buildingreqtext="Build your main building to level 3 to build a wall";}
		break;
	default:
	echo "error";
	unset($_GET["building"]);
...
$checky_checky_dwarf_2_building=$row['building'];
	...
	echo $name," ","level"," ",$level,"<img src=images/pil_upp.png>",$checky_checky_dwarf_2; ?><br><br><?	
	}else{
...
if ($level>=1 or $buildingreq==1 or empty($buildingreq)){	
	...
	if ($mysql_data["wood"]>$woodreq and $mysql_data["clay"]>$clayreq and $mysql_data["iron"]>$ironreq and $mysql_data["wheat"]>$wheatreq){?>
		<br>
		<a href="executables/func_start.php?building=<?php echo $upgrade; ?>"><img width="65" height="22" src="images/upgradebtn.png"></img></a><?
	}else{
